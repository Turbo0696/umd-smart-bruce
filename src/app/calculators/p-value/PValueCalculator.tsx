...
const DISTS: { value: Dist; label: string }[] = [
  { value: "z", label: "Z (standard normal)" },
  { value: "t", label: "T (Student's t)" },
];
...
export function PValueCalculator() {
  ...
  // Each mode keeps its own inputs, so toggling back and forth doesn't
  // lose what was typed.
  ...
  const isZ = dist === "z";
  const stat = parseFloat(isZ ? zInput : tInput);
  const df = parseFloat(dfInput);
  const valid = Number.isFinite(stat) && (isZ || (Number.isFinite(df) && df > 0));
  const p = !valid ? null : isZ ? zPValues(stat) : tPValues(stat, df);
  const pdf = isZ ? normalPdf : (x: number) => tPdf(x, df);
  const shown = p ? p[tail] : null;
  const tailLabel = TAILS.find((x) => x.value === tail)!.label;
  // Symbol for the statistic and its random variable: z / Z or t / T.
  const s = dist;
  const S = dist.toUpperCase();
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-12">
      ...
      <p className="mt-2 text-zinc-600 dark:text-zinc-400">
        {isZ ? (
          <>Enter a z-value for an exact p-value from the standard normal distribution.</>
        ) : (
          <>
      ...
      <div className="mt-6 flex flex-col gap-5 rounded-lg border border-zinc-200 p-5 dark:border-zinc-800">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex flex-wrap gap-1.5">
            {DISTS.map(({ value, label }) => (
              <button
                key={value}
                type="button"
                aria-pressed={dist === value}
                onClick={() => setDist(value)}
                className={segClass(dist === value)}
              >
                {label}
              </button>
            ))}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {isZ ? (
            <label className="flex flex-col gap-1">
              <span className="text-xs text-zinc-500">z-value</span>
              <input
                type="number"
                step="any"
                inputMode="decimal"
      ...
      <div className="mt-6 flex flex-col gap-5 rounded-lg border border-zinc-200 p-5 dark:border-zinc-800">
        ...
        {valid && (
          <div className="border-t border-zinc-200 pt-4 dark:border-zinc-800">
            <DensityCurve stat={stat} symbol={s} pdf={pdf} tail={tail} />
            <p className="mt-1 text-center text-xs text-zinc-500">
              {isZ ? "Standard normal distribution" : `t-distribution, df = ${df}`} —
              shaded area = {tailLabel.toLowerCase()} p-value
            </p>
          </div>
        )}
        ...
        {p && (
          <dl
            className={`grid grid-cols-2 gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-800 ${
              isZ ? "sm:grid-cols-3" : "sm:grid-cols-4"
            }`}
          >
            <Stat label={s} value={String(round3(stat))} />
            {!isZ && <Stat label="df" value={String(df)} />}
            <Stat label={`P(${S} ≤ ${s})`} value={fmtP(p.left)} />
            <Stat label={`P(${S} ≥ ${s})`} value={fmtP(p.right)} />
          </dl>
        )}
...
// Symmetric, zero-centred density with the p-value tail(s) shaded and the
...
function DensityCurve({
  ...
  const padT = 14;
  const baseline = H - 26;
  const domain = Math.max(4.2, Math.abs(stat) * 1.2 + 1.2);
  const maxY = pdf(0);
  const n = 140;
  const pts = Array.from({ length: n + 1 }, (_, i) => {
    const x = -domain + (2 * domain * i) / n;
    return [x, pdf(x)] as const;
  });
  ...
  // the shading meets the marker line even between sample points.
  function shade(a: number, b: number): string | null {
    if (a >= b) return null;
    const inner = pts.filter(([x]) => x > a && x < b);
    const edge = [[a, pdf(a)] as const, ...inner, [b, pdf(b)] as const];
    return (
  ...
  const abs = Math.abs(stat);
  const regions =
    tail === "two"
      ? [shade(abs, domain), shade(-domain, -abs)]
      : tail === "right"
        ? [shade(stat, domain)]
        : [shade(-domain, stat)];
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Distribution curve with the p-value tail region shaded"
    >
      ...
      />
      <line x1={px(stat)} y1={padT} x2={px(stat)} y2={baseline} strokeWidth={1.5} className="stroke-blue-600 dark:stroke-blue-400" />
      <text x={px(stat)} y={baseline + 16} textAnchor="middle" fontSize={12} className="fill-zinc-500 font-mono">
        {symbol}={round3(stat)}
      </text>
